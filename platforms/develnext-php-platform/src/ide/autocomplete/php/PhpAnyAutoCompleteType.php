...
class PhpAnyAutoCompleteType extends AutoCompleteType
{
    ...
    public function getStatements(AutoComplete $context, AutoCompleteRegion $region)
    {
        if (in_array($this->kind, ['~any'])) {
            ...
            foreach ($this->inspector->getSnippets() as $snippet) {
                $result[] = new StatementAutoCompleteItem(
                    $snippet->name,
                    $this->resolveSnippetDescription($snippet->description),
                    $snippet->code,
                    'icons/script16.png'
                );
            }
     ...
     * @return string
     */
    protected function resolveSnippetDescription($description)
    {
        if ($description === null) {
            return '';
        }

        $lang = Ide::get()->getLanguage()->getCode();
        ...
        if (is_array($description)) {
            ...
            if (isset($description['ru'])) {
                return (string) $description['ru'];
            }
        ...
        if (is_object($description)) {
            ...
            // TranslationText в IDE — stub без get(); не вызываем методы класса.
            if ($class === 'framework\\localization\\TranslationText'
                || str::endsWith($class, 'TranslationText')
            ) {
                $data = (array) $description;

                foreach (['', $lang, 'ru', 'en'] as $code) {
                    $key = "\0*\0value";
            ...
            $asArray = (array) $description;

            foreach ([$lang, '', 'ru', 'en'] as $code) {
                if ($code !== '' && isset($asArray[$code])) {
                    return (string) $asArray[$code];
                }
            }

            if (method_exists($description, '__toString')) {
                return (string) $description;
            }
        }
