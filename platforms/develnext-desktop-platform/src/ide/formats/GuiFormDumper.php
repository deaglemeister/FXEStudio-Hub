...
class GuiFormDumper extends AbstractFormDumper
{
     ...
     * @return UXForm
     */
    protected function getTestScene()
    {
    ...
    public function save(FormEditor $editor)
    {
        ...
        $testScene = $this->getTestScene();
        $testScene->clearStylesheets();
        $testScene->children->clear();

        foreach ($editor->getStylesheets() as $stylesheet) {
            $testScene->addStylesheet($stylesheet);
        }
    ...
    public function appendImports(array $nodes, DomDocument $document)
    {
        ...
        $import = $document->createProcessingInstruction('import', 'org.fxestudio.jphp.ext.gui.support.*');
        $document->insertBefore($import, $document->getDocumentElement());
                ...
                while ($class != null) {
                    ...
                    if ($class != null) {
                        ...
                        if ($tag != null) {
                            ...
                            $testNode = $tag->createTestNode($node, $this->getTestScene());
