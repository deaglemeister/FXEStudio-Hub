        ...
        foreach ([$this->gui->getFormEditors(), $this->gui->getModuleEditors(), $this->gui->getSpriteEditors()] as $i => $editors) {
            ...
            foreach ($editors as $editor) {
                ...
                if (FileSystem::isOpened($editor->getFile())) {
                    $menuItem->style = '-fx-text-fill: derive(-fx-text-base-color, 20%); -fx-font-weight: bold;';
                }
...
class GuiFrameworkProjectBehaviour extends AbstractProjectBehaviour
{
    ...
    protected function registerTreeMenu()
    {
        ...
        $tree->registerAddMenuCommand(new CreateFormProjectCommand($tree));
        $tree->registerAddMenuCommand(new CreateScriptModuleProjectCommand());
        $tree->registerAddMenuCommand(new CreateGameSpriteProjectCommand());
    }
    ...
    {
        $title = new UXLabel('Заставка');

        $label = new UXLabel('(Нет изображения)');
        $label->classes->add('fxe-project-muted-label');
        ...
        $selectRow = new UXHBox([$button, $label], 10);
        $selectRow->alignment = 'CENTER_LEFT';
        ...
        $left = new UXVBox([$selectRow, $fxSplashOnTop, $fxSplashAutoHide], 8);
        $left->fillWidth = true;
        UXHBox::setHgrow($left, 'ALWAYS');

        $preview = new UXVBox();
        $preview->classes->add('fxe-project-splash-preview');
        $preview->alignment = 'CENTER';

        $previewIcon = new UXLabel();
        $previewIcon->classes->add('fxe-project-splash-preview-icon');
        $preview->children->add($previewIcon);

        $body = new UXHBox([$left, $preview]);
        $body->spacing = 24;
        $body->alignment = 'TOP_LEFT';
        $body->fillHeight = false;

        $wrap = new UXVBox([$title, $body], 0);
    ...
    public function createModule($name)
    {
        ...
        Logger::debug("Finish creating module '$name'");
        ...
        public function createForm($name, $namespace = null)
    {
        ...
        Logger::debug("Creating form '$name' ...");
        ...
        Logger::debug("Finish creating form '$name'");
