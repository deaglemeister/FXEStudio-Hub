...
class DefaultGuiProjectTemplate extends AbstractProjectTemplate
{
    ...
    public function recoveryProject(Project $project)
    {
        ...
        // Проекты FXE Studio (по ideName из .conf) не нуждаются в миграциях
        // со старых DevelNext, их ideVersionHash имеет другой формат.
        if (!$this->isFxeProject($project)) {
    ...
    /**
     * Проект создан в FXE Studio? (по полю ideName из .dnproject/.conf)
     *
     * @param Project $project
     * @return bool
     */
    protected function isFxeProject(Project $project)
    {
        $ideName = (string) $project->getConfig()->getIdeName();

        return $ideName && str::contains(str::lower($ideName), 'fxe');
    }
    ...
    public function makeProject(Project $project)
    {
        ...
        $project->on('create', function () use ($gui, $bundle, $php, $project) {
            ...
            // Electron-подобная структура: src/app, src/assets, public.
            // src/.data — служебная папка рантайма (как resources в Electron).
            $project->makeDirectory('public');
            $project->makeDirectory('src/assets/images');
            $project->makeDirectory('src/assets/fonts');
            $project->makeDirectory('src/assets/sounds');
            $project->makeDirectory('src/.data/img');
            ...
            $project->getConfig()->setOpenedFiles([
                $project->getMainProjectFile(),
                $mainModule->getRelativePath(),
                $mainForm->getRelativePath(),
            ], $mainForm->getRelativePath());
        });
    ...
    {
        // Наш проект (FXE Studio, ideName из .conf) — миграция не нужна.
        if ($this->isFxeProject($project)) {
            return false;
        }
