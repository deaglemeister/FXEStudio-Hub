...
abstract class AbstractJarBundle extends AbstractBundle
{
    ...
    public function getProjectModules()
    {
        ...
        foreach ($this->getJarDependencies() as $dep) {
            ...
            if ($id) {
                $result[] = new ProjectModule(fs::abs("$id"), 'jarfile');
            }
        }
        ...
        foreach ($this->getProvidedJarDependencies() as $dep) {
            ...
            if ($id) {
                $result[] = new ProjectModule(fs::abs("$id"), 'jarfile', true);
            }
        }
    ...
    {
        parent::onLoad($project, $config);

        foreach ($this->getProjectModules() as $module) {
            if (!$project->hasModule($module, reflect::typeOf($this))) {
