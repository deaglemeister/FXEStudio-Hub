...
class FileSystem
{
    ...
    public static function getEditorSplitDividerWidth()
    {
        return static::$editorSplitDividerWidth;
    }

    public static function loadEditorSplitConfig()
    {
        $saved = (int) Ide::get()->getUserConfigValue('ide.editorLeftWidth', 250);
        if ($saved > 50) {
            static::$editorSplitDividerWidth = $saved;
        }
    }

    public static function saveEditorSplitConfig()
    {
        if (static::$editorSplitDividerWidth > 50) {
            Ide::get()->setUserConfigValue('ide.editorLeftWidth', (int) static::$editorSplitDividerWidth);
        }
    }
    ...
    static private function makeTabForEditor(AbstractEditor $editor)
    {
        ...
        $project = Ide::project();
        $filePath = $editor->getFile();

        uiLater(function () use ($tab, $project, $filePath, $editor) {
            $treeIcon = null;

            if ($project) {
                try {
                    $treeIcon = TreeIconResolver::loadForPath($project, $filePath);
                } catch (\Throwable $e) {
                    Logger::warn('Tab icon load failed: ' . $e->getMessage());
                }
            }

            if ($treeIcon) {
                $tab->graphic = $treeIcon;
            } elseif ($icon = $editor->getIcon()) {
                $tab->graphic = Ide::get()->getImage($icon, [16, 16]);
            }
        });
    ...
    /**
     * Закрыть текущую вкладку редактора (Ctrl+W).
     *
     * @return bool
     */
    public static function closeSelectedTab()
    {
        /** @var UXTabPane $fileTabPane */
        $fileTabPane = Ide::get()->getMainForm()->{'fileTabPane'};

        if (!$fileTabPane || !$fileTabPane->selectedTab) {
            return false;
        }

        $tab = $fileTabPane->selectedTab;

        if (!$tab->closable || !($tab->userData instanceof AbstractEditor)) {
            return false;
        }

        static::close($tab->userData->getFile());

        return true;
    }

    /**
     * Закрыть все открытые вкладки редактора проекта.
     *
     * @param bool $save
     */
    public static function closeAllOpenedTabs($save = true)
    {
        $paths = [];

        foreach (static::$openedTabs as $hash => $tab) {
            if ($editor = static::$openedEditors[$hash]) {
                $paths[] = $editor->getFile();
            }
        }

        foreach ($paths as $path) {
            static::close($path, true, $save);
        }
    }
    ...
    private static function showAddTab()
    {
        ...
        if (!static::$addTab) {
            ...
            $tab->closable = false;
            $tab->id = 'fxeWorkspaceAddTab';
            $tab->style = '-fx-cursor: hand; -fx-min-width: 28px; -fx-max-width: 28px; -fx-pref-width: 28px;';

            $button = new UXButton();
            $button->graphic = TreeIconResolver::loadIcon('openNewTab');
            $button->classes->add('fxe-workspace-add-tab-btn');
            $button->tooltipText = 'Новая вкладка';
            $tab->graphic = $button;
