...
class FormNamedBlock extends UXAnchorPane
{
    ...
    public function __construct($title, $icon)
    {
        ...
        $this->style = '-fx-border-width: 1px; -fx-border-color: #43454a; -fx-background-color: #26282c; -fx-border-radius: 3px; cursor: hand;';

        $label = new UXLabel($title);
        $label->id = 'title';
        $label->padding = [2, 5];
        $label->style = '-fx-background-color: #26282c; -fx-border-color: #43454a; cursor: hand; -fx-border-radius: 2px; -fx-text-fill: #b4b8bf;';
    ...
    public function setInvalid($value)
    {
        if ($value) {
            ...
            $this->label->style = str::replace($this->label->style, '-fx-text-fill: #b4b8bf', '-fx-text-fill: #ff6b68');
        } else {
            $this->iconNode->effects->clear();
            $this->label->style = str::replace($this->label->style, '-fx-text-fill: #ff6b68', '-fx-text-fill: #b4b8bf');
        }
    }
}