...
class IdeActionsPane extends UXHBox
{
    ...
    protected $zoomList = [10, 25, 33, 50, 67, 75, 100, 125, 150, 200, 300, 400, 500];
    ...
    public function setZoom($zoom)
    {
        ...
        $this->applyZoomUi($zoom);
        $this->designer->zoom = $this->designPane->zoom = $zoom / 100;
    }

    /**
     * Только обновить комбобокс и designer->zoom (без повторного setZoom на designPane).
     */
    public function setZoomUi($zoom)
    {
        if ($zoom < 10) {
            $zoom = 10;
        }
        ...
        $this->applyZoomUi($zoom);
        $this->designer->zoom = $zoom / 100;
    }

    protected function applyZoomUi($zoom)
    {
        $nearest = $this->zoomList[0];
        $minDiff = 9999;

        foreach ($this->zoomList as $item) {
            $diff = abs($item - $zoom);
