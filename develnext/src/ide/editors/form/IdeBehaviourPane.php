...
class IdeBehaviourPane
{
    ...
    public function makeUi($targetId, UXVBox $box = null)
    {
        ...
        if ($this->hintNode) {
            if ($this->hintNode instanceof UXTab || $this->hintNode instanceof UXLabeled) {
                $badgeCount = max(0, $box->children->count() - 1);

                if ($badgeCount) {
                    $this->hintNode->text = "";
                    $this->hintNode->graphic = $this->makeHintGraphic($badgeCount);
                } else {
                    $this->hintNode->graphic = null;
                    $this->hintNode->text = $this->hintNodeText;
                }
            } else {
    ...
    protected function makeHintGraphic($count = 0)
    {
        $parts = [new UXLabel($this->hintNodeText)];

        if ($count) {
            $countLabel = new UXLabel("+{$count}");
            $countLabel->classes->add('fxe-form-sidebar-tab-hint-badge');
            $parts[] = $countLabel;
        }

        $box = new UXHBox($parts);
        $box->classes->add('fxe-form-sidebar-tab-hint');
        $box->spacing = 4;
    ...
    protected function initDeleteBehaviourButton(UXTitledPane $pane, AbstractBehaviourSpec $spec, AbstractBehaviour $behaviour)
    {
        ...
        $button->on('click', function (UXEvent $e) use ($spec) {
        ...
        $scLab->on('click', function (UXEvent $e) use ($spec, $behaviour) {
            $model = [
