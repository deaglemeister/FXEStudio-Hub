...
use ide\editors\panels\FindReplaceBar;
...
use ide\systems\FxeCodeAnalysisSystem;
...
use php\gui\designer\UXLogCodeArea;
...
class CodeEditor extends AbstractEditor
{
     ...
     * @var bool
     */
    protected $hideReadOnlyBar = false;
    ...
    /**
     * @return UXCodeAreaScrollPane
     */
    public function getTextAreaScrollPane()
    {
    ...
    public function __construct($file, $mode = 'php', $options = array())
    {
        ...
        if ($textArea === null) {
            switch ($mode) {
                ...
                case 'log':
                    $this->textArea = new UXLogCodeArea();
                    break;
        ...
        $this->attachEditorContextMenu();
        $this->resetSettings();
    }

    /**
     * PHP-контекстное меню редактора (вместо Java Select All).
     */
    protected function attachEditorContextMenu()
    {
        ...
        $area->on('contextMenu', function ($e = null) use ($editor, $area) {
            $menu = new UXContextMenu();
            $menu->classes->add('fxe-editor-context-menu');

            $addItem = function ($title, $shortcut, $action) use ($menu) {
                $item = new UXMenuItem($title);
                $item->on('action', $action);
            ...
            $addItem('Отменить', 'Ctrl+Z', function () use ($area) {
                $area->undo();
            });
            $addItem('Повторить', 'Ctrl+Y', function () use ($area) {
                $area->redo();
            });
            $menu->items->add(UXMenuItem::createSeparator());
            $addItem('Вырезать', 'Ctrl+X', function () use ($area) {
                $area->cut();
            });
            $addItem('Копировать', 'Ctrl+C', function () use ($area) {
                $area->copy();
            });
            $addItem('Вставить', 'Ctrl+V', function () use ($area) {
                $area->paste();
            });
            $menu->items->add(UXMenuItem::createSeparator());
            $addItem('Выделить всё', 'Ctrl+A', function () use ($area) {
                $area->selectAll();
            });
            $menu->items->add(UXMenuItem::createSeparator());
            $addItem('Найти...', 'Ctrl+F', function () use ($editor) {
                $editor->executeCommand('find');
            });
            $addItem('Заменить...', 'Ctrl+H', function () use ($editor) {
                $editor->executeCommand('replace');
            });
            $addItem('Перейти к определению', 'F12', function () use ($editor) {
                FxeCodeAnalysisSystem::goToDefinition($editor);
            });

            $x = $e && isset($e->screenX) ? $e->screenX : 0;
            $y = $e && isset($e->screenY) ? $e->screenY : 0;
            $menu->show($area->form, $x, $y);
        });
    }
     ...
     * @param bool $hide
     */
    public function setHideReadOnlyBar($hide)
    {
        $this->hideReadOnlyBar = (bool) $hide;
    }
     ...
     */
    public function getFindReplaceBar()
    {
        if ($this->findReplaceBar) {
            return $this->findReplaceBar;
        }

        return $this->findReplaceBar = new FindReplaceBar($this);
    }
    ...
    public function executeCommand($command)
    {
        switch ($command) {
            ...
            case 'find':
                if ($this->textArea instanceof UXAbstractCodeArea) {
                    $this->getFindReplaceBar()->open(false);
                } else {
            ...
            case 'replace':
                if ($this->textArea instanceof UXAbstractCodeArea) {
                    $this->getFindReplaceBar()->open(true);
                } else {
    ...
    public function save()
    {
        ...
        Logger::debug("Start save file $this->file ...");

        if (!$this->contentLoaded) {
            Logger::debug("File '$this->file' skip saving, is not loaded to area yet.");
            return;
        }
        ...
        Logger::debug("Finish save file $this->file.");
        $this->fileTime = fs::time($this->file);
    }
    ...
    public function makeUi()
    {
        ...
        $label->textColor = 'red';
        $statusBar->classes->add('fxe-code-editor-readonly-bar');
        ...
        $editorArea = new UXAnchorPane();
        $editorArea->classes->add('fxe-code-editor-pane');
        UXAnchorPane::setAnchor($scrollPane, 0);
        $editorArea->add($scrollPane);

        if ($this->textArea instanceof UXAbstractCodeArea) {
            $findBarUi = $this->getFindReplaceBar()->getUi();
            $findBarUi->classes->add('fxe-find-bar-overlay');
            UXAnchorPane::setTopAnchor($findBarUi, 0);
            UXAnchorPane::setRightAnchor($findBarUi, 18);
            $editorArea->add($findBarUi);
        }

        $ui->add($editorArea);

        if ($this->isReadOnly() && !$this->hideReadOnlyBar) {
            $ui->add($statusBar);
        }

        UXVBox::setVgrow($editorArea, 'ALWAYS');
    ...
    {
        $value = Ide::get()->getUserConfigValue(__CLASS__ . '#' . $lang . '.highlight', 'VSCode-Dark');

        if (!self::getHighlightFile($lang, $value)->isFile()) {
            if (self::getHighlightFile($lang, 'VSCode-Dark')->isFile()) {
                return 'VSCode-Dark';
            }
