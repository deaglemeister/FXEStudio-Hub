...
use ide\editors\form\FormDesignerHistory;
use ide\editors\form\context\RedoDesignerMenuCommand;
use ide\editors\form\context\UndoDesignerMenuCommand;
...
use ide\systems\FormElementSafeRename;
...
class FormEditor extends AbstractModuleEditor
{
     ...
     * @var UXAnchorPane
     */
    protected $designerArea;

    /**
     * @var array
     */
    protected $bulkEditEntries = [];

    /**
     * @var FormDesignerHistory
     */
    protected $designerHistory;
    ...
    public function canUndoDesigner()
    {
        return $this->designerHistory && $this->designerHistory->canUndo();
    }

    public function canRedoDesigner()
    {
        return $this->designerHistory && $this->designerHistory->canRedo();
    }

    public function undoDesigner()
    {
        if ($this->designerHistory) {
            $this->designerHistory->undo($this);
        }
    }

    public function redoDesigner()
    {
        if ($this->designerHistory) {
            $this->designerHistory->redo($this);
        }
    }
     ...
     * @return string|null
     */
    public function getIdeConfigForHistory()
    {
        $config = $this->getIdeConfig();

        return $config ? $config->getFileName() : null;
    }

    public function restoreDesignerSnapshot(array $snapshot, array $fallbackSelection = [])
    {
        $this->restoringHistory = true;

        try {
            fs::ensureParent($this->getFxmlFile());
            Stream::putContents($this->getFxmlFile(), $snapshot['fxml']);

            $base = fs::pathNoExt($this->getCodeFile());

            if (isset($snapshot['axml'])) {
                fs::ensureParent($base . '.axml');
                Stream::putContents($base . '.axml', $snapshot['axml']);
            }

            if (isset($snapshot['code'])) {
                fs::ensureParent($this->getCodeFile());
                Stream::putContents($this->getCodeFile(), $snapshot['code']);
            }

            if (isset($snapshot['behaviour'])) {
                fs::ensureParent($base . '.behaviour');
                Stream::putContents($base . '.behaviour', $snapshot['behaviour']);
            }

            if (isset($snapshot['ideConf'])) {
                $ideConfigFile = $this->getIdeConfigForHistory();

                if ($ideConfigFile) {
                    fs::ensureParent($ideConfigFile);
                    Stream::putContents($ideConfigFile, $snapshot['ideConf']);
                }
            }

            $oldLayout = $this->layout;
            $parent = $oldLayout ? $oldLayout->parent : null;

            $this->formDumper->load($this);

            if ($parent && $oldLayout && $oldLayout !== $this->layout) {
                $parent->children->remove($oldLayout);
            ...
            if ($this->actionsPane && $ideConfig) {
                $this->actionsPane->setConfig($ideConfig->toArray());
            }

            $blockedNodes = $ideConfig ? $ideConfig->getArray('blockedNodes') : [];
            ...
            foreach ($this->designer->getNodes() as $node) {
                ...
                if ($nodeId && isset($blockedNodes[$nodeId])) {
            ...
            uiLater(function () use ($snapshot, $fallbackSelection) {
                if ($this->layout) {
                ...
                $selection = isset($snapshot['sel']) ? $snapshot['sel'] : $fallbackSelection;

                if (!$selection && $fallbackSelection) {
                    $selection = $fallbackSelection;
                }

                $this->restoreDesignerSelection($selection);
            });
        } finally {
    ...
    protected function attachDesigner()
    {
        $old = $this->designer;
        ...
        if ($old) {
            ...
            foreach ($old->getNodes() as $node) {
                $old->unregisterNode($node);
            }
        }
        ...
        if ($old) {
            $this->designer->snapType = $old->snapType;
            $this->designer->snapSizeX = $old->snapSizeX;
            $this->designer->snapSizeY = $old->snapSizeY;
            $this->designer->snapEnabled = $old->snapEnabled;
            $this->designer->zoom = $old->zoom;
            $this->designer->helpersEnabled = $old->helpersEnabled;
        }
        ...
        if ($this->designerArea) {
            $this->designer->addSelectionControl($this->designerArea);
        }
        ...
        if ($this->actionsPane) {
            $this->actionsPane->setDesigner($this->designer);
        }
    }
     ...
     * @return array
     */
    public function getDesignerSelectionIds()
    {
        if (!$this->designer) {
            return [];
        }
        ...
        $ids = [];
        $selected = $this->designer->getSelectedNodes();
        ...
        if (!$ids && $this->designer->pickedNode) {
            $id = $this->getNodeId($this->designer->pickedNode);
     ...
     * @param array $ids
     */
    public function restoreDesignerSelection(array $ids)
    {
        if (!$this->designer) {
            return;
        }

        waitAsync(100, function () use ($ids) {
            if ($this->layout) {
                $this->layout->applyCss();
            }
            ...
            waitAsync(150, function () use ($found, $firstId) {
                if ($this->layout) {
                    $this->layout->applyCss();
                }
                ...
                waitAsync(100, function () use ($found, $firstId) {
                    $this->designer->update();
                    ...
                    if ($found > 0) {
                        ...
                        if ($firstId) {
                            $this->leftPaneUi->refreshObjectTreeList($firstId);
                        }
                    } else {
                        $this->selectForm();
                    }
    ...
    public function save()
    {
        ...
        $this->saveFormFile();
        LivePropertySync::onFormSaved($this);
        /*if ($this->factory) {
            $this->factory->reload();
        }*/
    }
    ...
    public function open($param = null)
    {
        ...
        uiLater(function () {
            ...
            if ($this->designerHistory) {
                $this->designerHistory->reset($this);
            } else {
                $this->designerHistory = new FormDesignerHistory();
                $this->designerHistory->reset($this);
            }
    ...
    public function refreshLayout()
    {
        ...
        if ($this->viewerAndEvents) {
            $this->viewerAndEvents->requestLayout();
        }

        if ($this->codeEditor && method_exists($this->codeEditor, 'refreshUi')) {
            $this->codeEditor->refreshUi();
        }
    }
    ...
    public function validateNodeIdRename($node, $newId)
    {
    ...
    /**
     * Safe Rename: показать план изменений и выполнить переименование через RefactorSystem.
     *
     * @param UXNode $node
     * @param string $newId
     * @return string '' при успехе, 'invalid'|'busy'|'cancelled' при ошибке/отмене
     */
    public function requestSafeRename($node, $newId)
    {
        $result = $this->validateNodeIdRename($node, $newId);
        ...
        $oldId = $node->id;
        $plan = FormElementSafeRename::collect($this, $node, $newId);

        if (!SafeRenameForm::confirm($oldId, $newId, $plan)) {
            return 'cancelled';
        }

        return RefactorSystem::rename($this->getRefactorRenameNodeType(), $node, $newId);
    }
    ...
    public function changeNodeId($node, $newId)
    {
        ...
        $this->applySafeRenameInSources($oldId, $newId);
    ...
    /**
     * Обновить ссылки на id в PHP, CSS и других файлах проекта.
     */
    public function applySafeRenameInSources($oldId, $newId)
    {
        $phpFile = $this->getFile();

        if (fs::isFile($phpFile)) {
            $content = FileUtils::get($phpFile);
            $updated = FormElementSafeRename::replacePhpIdReferences($content, $oldId, $newId);

            if ($updated != $content) {
                FileUtils::put($phpFile, $updated);
                $this->eventManager->load();
            }
        }
        ...
        if (fs::isFile($actionFile)) {
            $content = FileUtils::get($actionFile);
            $pattern = '\b' . Regex::quote($oldId) . '\b';
            $updated = Regex::of($pattern)->with($content)->replace($newId);

            if ($updated != $content) {
                FileUtils::put($actionFile, $updated);
            }
        }

        foreach (FormElementSafeRename::collectCssFiles($this) as $cssFile) {
            if (!fs::isFile($cssFile)) {
                continue;
            }

            $content = FileUtils::get($cssFile);
            $updated = FormElementSafeRename::replaceCssIdReferences($content, $oldId, $newId);

            if ($updated != $content) {
                FileUtils::put($cssFile, $updated);
            }
        }
        ...
        if ($project) {
            $currentPhp = fs::abs(fs::pathNoExt($phpFile) . '.php');

            $project->eachSrcFile(function ($file) use ($oldId, $newId, $currentPhp, $phpFile) {
                $path = $file->getPath();
    ...
    {
        if (!$this->restoringHistory && $this->designerHistory) {
            $this->designerHistory->onBeforeChange($this);
        }
        ...
        if (!$this->restoringHistory) {
            ...
            if ($this->designerHistory) {
                $this->designerHistory->onAfterChange($this);
            }
        }
    }
    ...
    public function makeUi()
    {
        ...
        $tabs->classes->add('fxe-editor-view-tabs');
    ...
    protected function makeDesigner($fullArea = false)
    {
        ...
        if (!$fullArea) {
            ...
            $this->layout->style = '-fx-border-width: 0px; -fx-border-style: none; -fx-border-color: transparent;';
            $this->layout->backgroundColor = null;
        ...
        $scrollPane->maxWidth = 10000;
        $scrollPane->classes->add('fxe-component-palette-scroll');
        $this->elementTypePaneContainer = $scrollPane;

        $contentHost = new UXAnchorPane();
        UXAnchorPane::setAnchor($scrollPane, 0);
        $contentHost->children->add($scrollPane);
        $contentHost->minWidth = 180;
        UXVBox::setVgrow($contentHost, 'ALWAYS');

        $paletteRoot = new UXVBox();
        $paletteRoot->spacing = 8;
        $paletteRoot->classes->add('fxe-component-palette-shell');
        $paletteRoot->padding = [8, 8, 8, 8];
        $paletteRoot->fillWidth = true;

        if ($this->prototypeTypePane) {
            $protoScroll = new UXScrollPane($this->prototypeTypePane->getContent());
            $protoScroll->fitToWidth = true;
            $protoScroll->classes->add('fxe-component-palette-scroll');
            $protoScroll->hide();

            $modeSelect = new UXComboBox(['Объекты', 'Прототипы']);
            $modeSelect->maxWidth = 10000;
            $modeSelect->selectedIndex = 0;
            $modeSelect->classes->add('fxe-component-palette-mode');

            $modeSelect->on('action', function () use ($modeSelect, $scrollPane, $protoScroll, $contentHost) {
                $objects = $modeSelect->selectedIndex === 0;
                $scrollPane->visible = $objects;
                $scrollPane->managed = $objects;
                $protoScroll->visible = !$objects;
                $protoScroll->managed = !$objects;

                if (!$objects && !$contentHost->children->contains($protoScroll)) {
                    UXAnchorPane::setAnchor($protoScroll, 0);
                    $contentHost->children->add($protoScroll);
                }
            });
            ...
            $paletteRoot->children->addAll([$modeSelect, $contentHost]);
        } else {
            $paletteRoot->children->add($contentHost);
        }

        $palettePane = $paletteRoot;

        UXSplitPane::setResizeWithParent($palettePane, false);

        $actions = $this->makeActionsUi($designPane);

        if (!$fullArea) {
            $designPane->enableWheelZoom($viewer);
            $designPane->enableMiddleMousePan($viewer);
            $designPane->onZoomChanged(function ($zoomPercent) {
                if ($this->actionsPane) {
                    $this->actionsPane->setZoomUi((int) $zoomPercent);
                    $this->actionsPane->getEventHandler()->trigger('change');
                }
            });
        }
        ...
        if ($actions) {
            ...
            $split = new UXSplitPane([$wrap, $palettePane]);
        } else {
            $split = new UXSplitPane([$this->viewerAndEvents, $palettePane]);
        }

        $split->observer('width')->addOnceListener(function ($_, $width) use ($palettePane, $split, $scrollPane) {
            $class = __CLASS__;
            $saved = Ide::get()->getUserConfigValue("$class.paletteDivider");

            if ($saved !== null && $saved !== '') {
                $split->dividerPositions = [(float) $saved];
            } else {
                $split->dividerPositions = [1.0 - (240 / max(1, $width))];
            }

            $scrollPane->maxWidth = -1;
        });

        $scrollPane->observer('width')->addListener(function () use ($split) {
            $positions = $split->dividerPositions;
            if ($positions && count($positions) > 0) {
                Ide::get()->setUserConfigValue(__CLASS__ . '.paletteDivider', (float) $positions[0]);
            }
        });

        $split->observer('width')->addListener(function () use ($split) {
            $positions = $split->dividerPositions;
            if ($positions && count($positions) > 0) {
                Ide::get()->setUserConfigValue(__CLASS__ . '.paletteDivider', (float) $positions[0]);
            }
    ...
    protected function makeContextMenu()
    {
        ...
        $this->contextMenu->setCssClass('fxe-context-menu');
        $this->contextMenu->addSeparator();
        $this->contextMenu->add(new UndoDesignerMenuCommand());
        $this->contextMenu->add(new RedoDesignerMenuCommand());
        $this->contextMenu->addSeparator();
        $this->contextMenu->addCommand(AbstractCommand::makeWithText('События объекта', 'icons/wizard16.png', function () {
    ...
    protected function _onChanged()
    {
        ...
        LivePropertySync::onDesignerChanged($this);
    ...
    protected function _onSelectionChanged()
    {
        waitAsync(40, function () {
            $selected = $this->designer->getSelectedNodes();

            if ($selected && \sizeof($selected) > 1) {
                $this->updateBulkProperties($selected);
                return;
            }
    ...
    public function clearBulkEdit()
    {
        $this->bulkEditEntries = [];
    }

    public function isBulkEditMode()
    {
        return \sizeof($this->bulkEditEntries) > 1;
    }

    /**
     * @return array
     */
    public function getBulkEditEntries()
    {
        return $this->bulkEditEntries;
    }

    protected function setBulkEditNodes(array $nodes)
    {
        $this->bulkEditEntries = [];
        ...
        foreach ($nodes as $node) {
            ...
            $this->bulkEditEntries[] = [
                'node' => $node,
                'element' => $element,
                'target' => $element->getTarget($node),
            ];
        }
    }

    protected function updateBulkProperties(array $nodes)
    {
        $this->setBulkEditNodes($nodes);

        if (\sizeof($this->bulkEditEntries) < 2) {
            if ($this->bulkEditEntries) {
                $this->updateProperties($this->bulkEditEntries[0]['node']);
            }
            return;
        }
        ...
        $properties = BulkDesignHelper::buildProperties($this, $nodes);
        ...
        if ($properties && $properties->getGroupPanes()) {
            $this->trigger('updateNode:before', [$nodes, $properties]);

            if ($this->propertiesPane) {
                $this->propertiesPane->addProperties($properties);
            }

            $first = $this->bulkEditEntries[0];
            $count = \sizeof($this->bulkEditEntries);
            $title = $this->getNodeId($first['node']) . ' (+' . ($count - 1) . ')';

            $this->leftPaneUi->update($title, $first['target']);
        } else {
            $invalidLabel = new UXLabel('Нет общих свойств для выделенных компонентов');
    ...
    {
        Logger::debug("Initialize element = " . get_class($element));
    ...
    public function makeLeftPaneUi()
    {
        ...
        $objectTreeList->setRenameHandler(function ($nodeId, $newId) {
            $node = $this->layout->lookup("#$nodeId");

            if (!$node) {
                return 'invalid';
            }

            return $this->requestSafeRename($node, $newId);
        });
