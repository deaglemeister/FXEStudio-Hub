...
class SourceCodePreviewForm extends UXForm
{
    ...
    public function __construct($source, $syntax)
    {
        ...
        $commandPane->spacing = 8;
        $commandPane->minHeight = 40;
        $commandPane->classes->add('fxe-source-preview-footer');

        foreach ($commandPane->children as $i => $btn) {
            if ($i == 0) $btn->font->bold = true;

            $btn->paddingLeft = $btn->paddingRight = 15;
            $btn->classes->add('fxe-source-preview-btn');
        }

        $box = new UXVBox([$node, $commandPane], 0);
        $box->padding = [10, 10, 10, 10];
        $box->classes->add('fxe-source-preview');
