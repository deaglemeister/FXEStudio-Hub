...
class IdeLibrary
{
    ...
    public function __construct(Ide $ide)
    {
        ...
        $this->defaultDirectory = File::of("$home/FXEStudioLibrary");

        if ($ide->isSnapshotVersion()) {
            $this->defaultDirectory = File::of("$home/FXEStudioLibrary.{$ide->getVersionHash()}.SNAPSHOT");
        }
    ...
    public function updateCategory($code)
    {
        ...
        if ($type = $this->categories[$code]) {
            ...
            foreach ($directories as $directory) {
                Logger::debug("Scan directory: $directory/$code");
                ...
                $filter = [
                    ...
                    'callback' => function ($filename) use ($code, $type) {
                        ...
                        Logger::debug("Add library ($code) resource $filename, type = $type[type]");
    ...
    public function update()
    {
        ...
        foreach ($this->categories as $code => $type) {
            ...
            $counts[$code] = count((array)$this->resources[$code]);
        }

        $elapsed = Time::millis() - $started;
        Logger::info(
            "Library scan done: projects={$counts['projects']}, bundles={$counts['bundles']}, generators={$counts['scriptGenerators']}, skins={$counts['skins']}, time={$elapsed}ms"
        );
    }
