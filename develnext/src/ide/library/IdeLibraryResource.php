...
abstract class IdeLibraryResource implements MenuViewable
{
    ...
    public function __construct($path = null)
    {
        ...
        if ($path) {
            $resourceFile = $path . '.resource';
            try {
                $this->config = new Configuration();
                if (fs::isFile($resourceFile)) {
                    $this->config->load($resourceFile);
                }

                // metadata-файл опциональный, сам ресурс считается валидным.
                $this->valid = true;
            } catch (IOException $e) {
                $this->config = new Configuration();
                Logger::warn("Unable to read '{$resourceFile}' file - " . $e->getMessage());
                $this->valid = false;
            }
        } else {
            $this->config = new Configuration();
