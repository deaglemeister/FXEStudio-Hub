...
class AutoCompletePane
{
        ...
        $this->area->on('keyUp', function (UXKeyEvent $e) {
            ...
            if (!$hintShown) {
                if ($string = $this->getString()) {
                    ...
                    $applyList = function ($items) use ($x, $y) {
                        uiLater(function () use ($x, $y, $items) {
                            if ($items !== null) {
                    ...
                    if ($items !== null) {
                        ...
                        if (FxeProcessSystem::getProcess('lsp')) {
                            $content = $this->area->text;
                            $line = $this->area->caretLine;
                            $column = $this->area->caretOffset;

                            FxeLanguageServerClient::requestCompletions($content, $line, $column, function ($lspItems) use ($items, $applyList) {
                                if (!$lspItems) {
                                    return;
                                }

                                $merged = FxeLanguageServerClient::mergeCompletionItems($items, $lspItems);
                                $applyList($merged);
                            });
                        }
                    } else {
                        uiLater(function () use ($x, $y) {
                            if ($this->list->items->count) {
                                $this->show($x, $y);
                            } else {
                                $this->hide();
    ...
    private function makeUi()
    {
        ...
        $list->classes->addAll(['hide-hor-scroll', 'dn-console-list', 'dn-autocomplete', 'fxe-monaco-complete']);
        $list->width = 400;
    ...
    /** Monaco / VS Code Dark+ autocomplete item colors */
    const COLOR_DEFAULT = '#B4B8BF';
    const COLOR_HINT = '#808080';
    const COLOR_VARIABLE = '#9876AA';
    const COLOR_METHOD = '#FFC66D';
    const COLOR_PROPERTY = '#9876AA';
    const COLOR_CONSTANT = '#6897BB';
    const COLOR_CLASS = '#FFC66D';

    protected function makeItemUi(UXListCell $cell, AutoCompleteItem $item)
    {
        $label = new UXLabel($item->getName());
        $label->textColor = UXColor::of(self::COLOR_DEFAULT);
        ...
        if (!$item->getDescription()) {
            ...
            $hintLabel->textColor = UXColor::of(self::COLOR_HINT);
        ...
        if ($item instanceof VariableAutoCompleteItem) {
            ...
            $label->textColor = UXColor::of(self::COLOR_VARIABLE);
        }

        if ($item instanceof MethodAutoCompleteItem) {
            $label->text = "{$label->text}()";
            $label->textColor = UXColor::of(self::COLOR_METHOD);
        }

        if ($item instanceof PropertyAutoCompleteItem) {
            $label->text = "{$label->text}";
            $label->textColor = UXColor::of(self::COLOR_PROPERTY);
        }

        if ($item instanceof ConstantAutoCompleteItem) {
            $label->textColor = UXColor::of(self::COLOR_CONSTANT);
        }
    }
}