...
class ServiceManager
{
    ...
    protected $lastServiceWarnAt = 0;

    /**
     * Онлайн-сервисы develnext.org давно не работают, поэтому по умолчанию
     * менеджер выключен и не шлёт запросы. Включить можно через
     * user config: account.service.enabled = 1 или -Ddevelnext.endpoint=...
     * @var bool
     */
    ...
    public function __construct()
    {
        ...
        $this->enabled = Ide::get()->getUserConfigValue('account.service.enabled', '0') == '1'
            || !!System::getProperty('develnext.endpoint');
        ...
        if ($this->enabled) {
            ...
            Logger::debug("Account service is disabled (account.service.enabled = 0)");
        }
    }
    ...
    public function updateStatus()
    {
        ...
        if ($this->ideService) {
            if (Ide::get()->isIdle()) {
                Logger::debug("Skip update status, ide in idle mode ...");
                return;
            }

            Logger::debug("Update status request, endpoint = {$this->getEndpoint()}");

            $this->ideService->statusAsync(function (ServiceResponse $response) {
                $isUnavailable = $response->isConnectionRefused() || $response->isConnectionFailed();

                if ($isUnavailable) {
                    $now = Time::millis();
                    $canWarn = ($now - $this->lastServiceWarnAt) >= 5 * 60 * 1000;
                    $message = "Account service unavailable: {$response->message()}";

                    if ($canWarn) {
                        Logger::warn($message);
                        $this->lastServiceWarnAt = $now;
                    } else {
                        Logger::debug($message);
                    }
                } else {
                    Logger::debug("Update status response = {message: {$response->message()}, data: " . Json::encode($response->result()) . "}");
                }

                if ($response->isConnectionRefused() || $response->isConnectionFailed()) {
                    $this->changeStatus([
                        'private' => 'fail',
                        'public' => 'fail',
                    ]);
