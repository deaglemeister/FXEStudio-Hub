...
class UiUtils
{
    ...
    static function makePropertyExpandButton(callable $onClick)
    {
        $button = new \php\gui\UXButton();
        $button->classes->add('fxe-property-expand-btn');
        $button->minWidth = 22;
        $button->prefWidth = 22;
        $button->maxWidth = 22;
        $button->minHeight = 22;
        $button->prefHeight = 22;
        $button->maxHeight = 22;
        $button->padding = [2, 2];
        $button->text = '';
        $button->graphic = \ide\ui\FxeTitlebarIcons::graphic(\ide\ui\FxeTitlebarIcons::NEW_TAB, [14, 14]);
    ...
    static function checkIO($message, $important = true)
    {
        if (UXApplication::isUiThread()) {
            if ($important) {
                Logger::warn("IO operation in UI Thread, $message");
            } else {
                Logger::debug("IO operation in UI Thread, $message");
            }
        }
    }
