...
class FileUtils
{
     ...
     * @return string|null
     */
    public static function getHead($file, $limit = 8192)
    {
    ...
    public static function copyFile($origin, $dest)
    {
        ...
        try {
            ...
            if (UXApplication::isUiThread()) {
                if ($time > 150) {
                    Logger::warn("Slow copy file [$time ms] in UI thread, $origin -> $dest");
                } elseif ($time > 10) {
                    UiUtils::checkIO("{$time}ms, copyFile(): $origin -> $dest", false);
                }
            }
        }
    }
    ...
    public static function deleteDirectory($directory)
    {
        ...
        if ($time > 10) {
            UiUtils::checkIO("{$time}ms, deleteDirectory(): $directory", $time > 100);
        }
    ...
    public static function put($filename, $content, $encoding = 'UTF-8')
    {
        ...
        try {
            ...
            $time = Time::millis() - $time;
            if ($time > 10) {
                UiUtils::checkIO("{$time}ms, put(): $filename", $time > 100);
            }
        }
    }
    ...
    public static function get($filename, $encoding = 'UTF-8')
    {
        try {
            ...
            if ($time > 10) {
                UiUtils::checkIO("{$time}ms, get(): $filename", $time > 100);
            }
