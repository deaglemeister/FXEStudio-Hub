...
class Project
{
    ...
    public function open()
    {
        ...
        $this->getTree()->completeContextMenu();
        ...
        // Реиндекс откладываем: сначала показываем окно проекта,
        // индексация пройдёт после первого кадра (короткая пауза вместо фриза при открытии).
        waitAsync(600, function () {
            uiLater(function () {
                if (Ide::get()->getOpenedProject() === $this) {
                    $this->reindex();
                }
            });
        });

        $this->openConfiguredFiles();
    ...
    /**
     * Открывает ранее открытые файлы проекта по одному за кадр UI,
     * чтобы не фризить интерфейс при большом числе вкладок, затем
     * выделяет ранее выбранный файл.
     */
    protected function openConfiguredFiles()
    {
        $filesToOpen = [];
        ...
        $openNext = function () use (&$openNext, &$filesToOpen, $selected, $project) {
    ...
    {
        $time = Time::millis();
        Logger::debug("Refresh project supports ...");
        ...
        foreach ($supports as $key => $support) {
            if (!$support->isFit($this)) {
                Logger::debug("Unlink support '$key' from project");
        ...
        foreach ($ide->getProjectSupports() as $support) {
            ...
            if (!isset($this->supports[$cls]) && $support->isFit($this)) {
                Logger::debug("Link support '$cls' to project");
                $support->onLink($this);
        ...
        Logger::debug("Refreshing project supports is done, time = {$time}ms.");
    }
    ...
    /**
     * Тяжёлая загрузка без UI (фоновый поток).
     */
    public function loadBackground()
    {
        Logger::debug("Project loading (background) ...");
    ...
    /**
     * UI-часть загрузки: inject поведений, меню дерева.
     */
    public function loadUi()
    {
        Logger::debug("Project loading (ui) ...");

        foreach ($this->behaviours as $behaviour) {
            Logger::debug("Inject behaviour: " . reflect::typeOf($behaviour));
            $behaviour->inject();
        }
