...
class ShareProjectBehaviour extends AbstractProjectBehaviour
{
    ...
    {
        if ($this->uiSettings) {
            $this->uiSettings->children->clear();

            if (Ide::accountManager()->isAuthorized()) {
                $this->uiSettings->children->add($this->uiSyncPane);
                ...
                $this->uiSettings->children->add($this->uiAuthPane);
            }
        }
    }
    ...
    public function doMakeSettings(CommonProjectControlPane $editor)
    {
        ...
        $this->uiSettings = $editor->getSyncHost();
    }
}