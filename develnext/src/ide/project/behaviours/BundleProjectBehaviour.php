...
class BundleProjectBehaviour extends AbstractProjectBehaviour
{
    ...
    public function inject()
    {
        ...
        foreach ($this->getPublicBundles(true) as $bundle) {
            $vendorDir = $bundle->getVendorDirectory();
            $hash = FileUtils::hashName($vendorDir);

            if (!isset($addedPaths[$hash])) {
                $addedPaths[$hash] = FileUtils::normalizeName($vendorDir);
                IdeSystem::getLoader()->addClassPath($vendorDir);
            }
        }

        Logger::info("Added bundle class paths: " . sizeof($addedPaths));
        Logger::debug("Bundle class paths: " . str::join(array_values($addedPaths), ', '));
    ...
    public function doPreCompile($env, callable $log = null)
    {
        ...
        if ($result['error']) {
            ...
            if ($log) {
                $log(':warn unable to clean src_generated, files locked? ' . str::join($failed, ', '));
            }
