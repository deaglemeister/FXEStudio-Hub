...
class ProjectTree
{
     ...
     * @var bool
     */
    protected $contextMenuCompleted = false;
    ...
    public function __construct(Project $project)
    {
        ...
        $this->contextMenu->addGroup('add', 'Добавить', 'icons/plus16.png');

        $this->contextMenu->add(new TreeCreateFileCommand($this), 'add');
        $this->contextMenu->add(new TreeCreateDirectoryCommand($this), 'add');
    }

    /**
     * Регистрация пунктов подменю «Добавить» из платформ (PHP, GUI и т.д.).
     */
    public function registerAddMenuCommand(AbstractMenuCommand $command)
    {
        $this->contextMenu->add($command, 'add');
    }

    public function registerAddMenuSeparator()
    {
        $this->contextMenu->addSeparator('add');
    }

    protected function finishContextMenu()
    {
    ...
    /**
     * Вызывается после регистрации behaviour-меню.
     */
    public function completeContextMenu()
    {
        if ($this->contextMenuCompleted) {
            return;
        }

        $this->finishContextMenu();
        $this->contextMenuCompleted = true;
    }
     ...
     * @return bool
     */
    public function isProtectedPath($path)
    {
        if (!$path) {
            return true;
        }

        if (PhpStructureTreeProvider::isVirtualPath($path)) {
            return true;
        }

        return fs::ext($path) === 'dnproject';
    }

    /**
     * @return bool
     */
    public function canModifySelectedPath()
    {
        if (!$this->hasSelectedPath()) {
            return false;
        }

        return !$this->isProtectedPath($this->getSelectedPath());
    }
    ...
    public function setView(UXDirectoryTreeView $treeView)
    {
        ...
        $treeView->on('click', function (UXMouseEvent $e) use ($treeView) {
            if ($e->clickCount > 1) {
                ...
                if ($selectedPath) {
                    if (PhpStructureTreeProvider::isVirtualPath($selectedPath)) {
                        foreach ($this->openHandlers as $handler) {
                            if ($handler($this->project->getRootDir())) {
                                return;
                            }
                        }
                    ...
                    if ($file->isFile()) {
                        ...
                        if (!$editor) {
                            switch (fs::ext($file)) {
                                ...
                                default:
                                    Ide::toast("Не удалось открыть файл: " . fs::name($file));
                            }
    ...
    /**
     * Те же правила, что и в дереве проекта — для подсчёта файлов в статус-баре.
     *
     * @param string|\php\io\File $file
     * @return bool
     */
    public function isVisiblePath($file)
    {
        if (!$this->project) {
            return false;
        }
        ...
        $path = FileUtils::relativePath($this->project->getRootDir(), $file);
    ...
    {
        $root = $this->project->getRootDir();
        $dirs = 0;
        $files = 0;

        foreach (static::scanRoots($root) as $scanRoot) {
            if (!fs::isDir($scanRoot)) {
                continue;
            }

            fs::scan($scanRoot, function ($path) use (&$dirs, &$files) {
                if (!static::isRelevantProjectPath($path) || !$this->isVisiblePath($path)) {
                    return;
                }
     ...
     * @return string[]
     */
    public static function scanRoots($root)
    {
        return [
            fs::normalize($root . '/public'),
            fs::normalize($root . '/src/assets'),
            fs::normalize($root . '/src/app/forms'),
            fs::normalize($root . '/src/app/modules'),
            fs::normalize($root . '/src/.theme'),
            fs::normalize($root . '/src/.data'),
        ];
    }
     ...
     * @return bool
     */
    public static function isRelevantProjectPath($path)
    {
        $name = fs::name($path);

        if ($name === '.debug' || $name === '.dn' || $name === 'vendor' || $name === 'node_modules') {
            return false;
        }
    ...
    public function createSource()
    {
        ...
        $source->addValueCreator(function ($path, File $file) use ($project) {
            return TreeIconResolver::createValue($project, $path, $file);
        });

        PhpStructureTreeProvider::register($source, $this->project);
        PhpStructureTreeProvider::registerOpenHandler($this, $this->project);
