...
class Ide extends Application
{
     ...
     * @return UXImageView
     */
    public static function getImage($path, array $size = null, $cache = true,bool $customicon = false)
    {
        if ($path === null) {
            return null;
        }
        ...
        if ($path instanceof UXImage) {
            ...
            if ($cache) {
                if($customicon){
                    $image = Cache::getResourceImage($path);
                }else{
