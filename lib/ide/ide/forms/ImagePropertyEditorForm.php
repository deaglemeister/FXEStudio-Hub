            ...
            foreach ($files as $file) {
                if ($project->findDuplicatedFiles($file)) {
                    if (!$showed) {
                        ...
                        $tm
                        ->setIcon($iconImage)
                        ->setTitle('Менеджер по работе с изображениями')
                        ->setDescription(_('Данная картинка уже находится в вашем текущем проекте.'))
                        ->setClosable();
                        Toaster::show($tm);
                        $showed = true;
                    }
    ...
    public function actionLoad()
    {
        if ($file = $this->dialog->execute()) {
            ...
            if ($files) {
                ...
                $this->show_tc(['message' => "Изображение уже есть в проекте, будет использован оригинал из проекта", 'color' => 'red']);
            } else {
                $this->setResult($file);
            }
        }
