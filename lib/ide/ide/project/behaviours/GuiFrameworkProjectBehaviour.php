...
class GuiFrameworkProjectBehaviour extends AbstractProjectBehaviour
{
    ...
    public function doUpdateSettings(CommonProjectControlPane $editor = null)
    {
        if ($this->uiSplashLabel) {
            $this->uiSplashLabel->text = $this->splashData['src'] ?: _('no.img');
        }
    ...
    public function doMakeSettings(CommonProjectControlPane $editor)
    {
        ...
        $button->on('action', function () use ($label) {
            ...
            if ($dialog->showDialog()) {
                ...
                $label->text = $this->splashData['src'] ?: _('no.img');
        ...
        if(fs::exists($this->project->getRootDir().'\.dn\ide\project\behaviours\FXECustomIcon.png')){
            ...
            $iconTextPath->text = 'Иконка для данного проекта отсутствует';
        }
        ...
        $chooseIconButton->on('action', function () use ($iconTextPath) {
            ...
            try {
                $ImgPath = new FileChooserScript();
                $ImgPath->filterExtensions = '*.png';
                $img = $ImgPath->execute();
                $pathFoldr = $this->project->getRootDir().'\.dn\ide\project\behaviours';
                if(!fs::exists($pathFoldr)){
                    fs::makeDir($pathFoldr);
                }
        ...
        $iconBox = new UXHBox([$chooseIconButton, $iconTextPath]); 
        $iconBox->spacing = 10;
        $iconBox->alignment = 'CENTER_LEFT';

        $wrapIcon = new UXVBox([$titleIcon,$iconBox]);
        $wrapIcon->spacing = 5;

        $editor->addSettingsPane($wrapIcon);
