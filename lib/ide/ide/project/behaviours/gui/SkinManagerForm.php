...
class SkinManagerForm extends AbstractIdeForm
{
    ...
    public function init()
    {
        ...
        $contextMenu = new ContextMenu(null, [
            SimpleSingleCommand::makeWithText(_('save.skin.f13'), 'icons/ok16.png', function () {
                $this->doSelect();
            }),
            '-',
            SimpleSingleCommand::makeWithText(_('save.skin.f12'), 'icons/save16.png', function () {
        ...
        $this->list->setCellFactory(function (UXListCell $cell, ?IdeLibrarySkinResource $resource) {
            if ($resource) {
                ...
                if ($skin = $resource->getSkin()) {
                    ...
                    if ($skin->getAuthor()) {
                        if ($desc) $desc .= ", ";
                        $desc .= _('save.skin.f7') . $skin->getAuthor();
                    }
                    ...
                    if ($skin->getVersion()) {
                        if ($desc) $desc .= ", ";
                        $desc .= "version: {$skin->getVersion()}";
                    }

                    if (!$desc) {
                        $desc = _('save.skin.f15');
                    }
                ...
                $cell->text = null;
                $cell->graphic = $ui = new ListExtendedItem(_('save.skin.f16'), _('save.skin.f17'), ico('brush16'));
