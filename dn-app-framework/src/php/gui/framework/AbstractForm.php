...
abstract class AbstractForm extends UXForm
{
    ...
    {
        $layout = $this->layout;
        $preloader = Preloader::getPreloader($layout);

        if ($preloader) {
            $preloader->setText($text);
        } else {
            $preloader = new Preloader($layout, $text);
        }
    ...
    /**
     * Показать preloader, дать UI отрисоваться и выполнить тяжёлую работу в фоне.
     *
     * @param string $text
     * @param callable $work
     * @param callable|null $after
     */
    public function runWithPreloader($text, callable $work, callable $after = null)
    {
        $this->showPreloader($text);

        waitAsync(48, function () use ($work, $after) {
            ThreadPool::createCached()->execute(function () use ($work, $after) {
                $result = null;
                $error = null;
                ...
                uiLater(function () use ($after, $result, $error) {
                    $this->hidePreloader();
