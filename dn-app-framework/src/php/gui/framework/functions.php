...
/**
 * --RU--
 * Выполнить колбэк после короткой паузы, чтобы JavaFX успел отрисовать UI.
 * Используйте между showPreloader() и тяжёлой синхронной работой.
 *
 * @param callable $callback
 * @param int $delayMs
 */
function uiAfterPulse(callable $callback, $delayMs = 48)
{
    waitAsync($delayMs, $callback);
}
