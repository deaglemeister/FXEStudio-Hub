...
public class UXAbstractCodeArea<T extends AbstractCodeArea> extends UXRegion<AbstractCodeArea> {
    public static class EventProvider extends org.develnext.jphp.ext.javafx.support.EventProvider<AbstractCodeArea> {
        public EventProvider() {
            ...
            setHandler("colorPick", new Handler() {
                @Override
                public void set(AbstractCodeArea target, EventHandler eventHandler) {
                    target.setOnColorPick(eventHandler);
                }

                @Override
                public EventHandler get(AbstractCodeArea target) {
                    return target.getOnColorPick();
                }
            });

            setHandler("contextMenu", new Handler() {
                @Override
                public void set(AbstractCodeArea target, EventHandler eventHandler) {
                    target.setUseCustomContextMenu(eventHandler != null);
                    target.setOnContextMenuRequest(eventHandler);
                }

                @Override
                public EventHandler get(AbstractCodeArea target) {
                    return target.getOnContextMenuRequest();
                }
            });
        }
    ...
    public void appendText(String text, String styleClass) {
        ...
        AbstractCodeArea area = getWrappedObject();

        if (area instanceof org.develnext.jphp.gui.designer.editor.syntax.impl.LogCodeArea) {
            ((org.develnext.jphp.gui.designer.editor.syntax.impl.LogCodeArea) area).appendLogText(text, styleClass);
            return;
        }
        ...
        if (styleClass != null && !styleClass.trim().isEmpty() && end > start) {
            try {
                area.setStyle(start, end, Collections.singleton(styleClass));
    ...
    @Signature
    public void end() {
        int len = getWrappedObject().getLength();
        getWrappedObject().moveTo(len);
        getWrappedObject().requestFollowCaret();

        int lastParagraph = getWrappedObject().getParagraphs().size() - 1;

        if (lastParagraph >= 0) {
            getWrappedObject().showParagraphAtBottom(lastParagraph);
        }
    }
}
